...
wire [31:0] branch_target_ID; // MỚI: Wire chứa địa chỉ nhảy tính tại ID

wire [31:0] PC_EX, PC_plus4_EX, dataA_EX, dataB_EX, imm_ext_EX;
wire [4:0] addA_EX, addB_EX, addD_EX;
wire [2:0] funct3_EX;
wire [6:0] funct7_EX;         // MỚI: Wire đồng bộ funct7 sang EX
wire [6:0] op_EX;             // MỚI: Wire đồng bộ opcode sang EX
...
IMEM imem(.PC_Out(PC_Out), .instruction(instruction));

assign PC_IF = (stall) ? PC_Out : PC_plus4; // Stall giữ PC

// SỬA LỖI: Lấy địa chỉ nhảy (branch_target_ID) được tính sớm ở tầng ID, không lấy từ EX nữa
NextPCSel next_pc_sel(
    .PC_plus4(PC_plus4), 
    .PC_branch(branch_target_ID), // <-- Đã sửa
    .Branch(Branch_ID),           // <-- Đã sửa (Dùng Branch_ID vì xét ở ID)
    .Jump(Jump_ID),               // <-- Đã sửa
...
IF_ID if_id(
    .clk(clk), .rst(rst), .stall(stall), .flush(flush_ID),
...
ControlUnit control_unit(
    .op(instruction_ID[6:0]), .RegWrite(RegWrite_ID), .ALUSrc(ALUSrc_ID),
...
RegisterFile reg_file(
    .clk(clk), .rst(rst), .addA(instruction_ID[19:15]), .addB(instruction_ID[24:20]),
...
BranchComp branch_comp(
    .op(instruction_ID[6:0]), .funct3(instruction_ID[14:12]),
...
// SỬA LỖI: Thêm bộ cộng tính địa chỉ nhảy nhánh ngay tại tầng ID
assign branch_target_ID = PC_ID + imm_ext_ID;
...
ID_EX id_ex(
    ...
    .funct3_in(funct3_ID), 
    .funct7_in(instruction_ID[31:25]), .funct7_out(funct7_EX), // <-- MỚI: Đồng bộ funct7
    .op_in(instruction_ID[6:0]), .op_out(op_EX),               // <-- MỚI: Đồng bộ opcode
    .RegWrite_in(RegWrite_ID), .ALUSrc_in(ALUSrc_ID),
...
//                 EX STAGE
// ==========================================
// SỬA LỖI: Dùng funct7_EX và op_EX thay vì kéo thẳng từ instruction_ID
ALUControl alu_control(
    .ALUOp(ALUOp_EX), .funct3(funct3_EX), 
    .funct7(funct7_EX), // <-- Đã sửa
    .op(op_EX),         // <-- Đã sửa
...
MUX3 mux_forwardA(
    .input0(dataA_EX), .input1(WB_out_WB), .input2(alu_out_MEM),
    .select(forwardA), .out(forwardA_data)
);

MUX3 mux_forwardB(
    .input0(dataB_EX), .input1(WB_out_WB), .input2(alu_out_MEM),
...
ForwardingUnit forwarding_unit(
    .rs1_EX(addA_EX), .rs2_EX(addB_EX), .rd_MEM(addD_MEM), .rd_WB(addD_WB), 
...
// ==========================================
MUX3 mux_wb(
    .input0(alu_out_WB), .input1(read_data_WB), .input2(PC_plus4_WB),
    .select(ResultSrc_WB), .out(WB_out_WB)
);

// --- Hazard Detection Unit ---
HazardDetectionUnit hazard_detection(
    .rs1_ID(addA_ID), .rs2_ID(addB_ID), .rd_EX(addD_EX), .rd_MEM(addD_MEM), // Thêm rd_MEM
    .MemRead_EX(MemRead_EX), 
    .RegWrite_EX(RegWrite_EX), .RegWrite_MEM(RegWrite_MEM),                 // Thêm RegWrite
    .branch_taken_ID(branch_taken_ID), .Branch_ID(Branch_ID), .Jump_ID(Jump_ID),
